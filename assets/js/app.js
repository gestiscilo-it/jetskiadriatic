...
/* ==============================================================
   PINNED HORIZONTAL SCROLL — Love + Gallery
   Each .hscroll-pin section is taller than the viewport. Inside,
   a sticky stage stays pinned while the user scrolls; the track
   inside is translateX-ed in proportion to scroll progress.
   On viewports under 900px we bail and let the native scroll snap
   in the CSS take over.
============================================================== */
(() => {
  const sections = document.querySelectorAll('.hscroll-pin');
  ...
  function measure(section){
    ...
    const portW  = viewport.clientWidth;
    return { track, viewport, maxTranslate: Math.max(0, trackW - portW) };
  }

  function setHeights(){
    sections.forEach(section => {
      // On mobile, native scroll takes over — clear pin sizing.
      if (MOBILE()){
      ...
      if (!m) return;
      // Vertical scroll distance = horizontal travel (1:1) + one viewport for the pin.
      section.style.height = `calc(100vh + ${m.maxTranslate}px)`;
    });
    update();
  }
  ...
  function update(){
    ...
    sections.forEach(section => {
      ...
      m.track.style.transform = `translate3d(${(-p * m.maxTranslate).toFixed(1)}px, 0, 0)`;
      const bar = section.querySelector('.hscroll-progress-bar');
  ...
  window.addEventListener('resize', () => {
    ...
    resizeT = setTimeout(setHeights, 120);
  });

  // Re-measure once images load — card heights affect track size.
